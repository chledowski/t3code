...
Choose the other account from the thread's model picker. Codex instances that
share the thread's **CODEX_HOME path** continue the conversation as it is. An
instance with a different home cannot read it, so T3 Code asks before switching
and the thread starts fresh from the next message there; the transcript stays.
